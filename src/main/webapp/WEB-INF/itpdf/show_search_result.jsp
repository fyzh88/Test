...
<body>
	...
	<c:set var="total" value="${fn:length(requestScope.search_result)}"></c:set>
	...
	<div class="row">
		<c:forEach var="colIndex"  begin="1" end="${colCnt }" step ="1">
			...
			<c:if test="${total>index }">
				<div class="col-md-3">
					<a href="show_pdf?bookId=${requestScope.search_result[index]['id'] }">
					<img src="getPdfImageOf?bookId=${requestScope.search_result[index]['id'] }&pageIndex=0" 
					           class="img-thumbnail" alt="${requestScope.search_result[index]['name'] }">
						${requestScope.search_result[index]['name'] }
					</a>
				</div>
			</c:if>
		</c:forEach>
